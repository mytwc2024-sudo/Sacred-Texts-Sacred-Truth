...
const CORS = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-oracle-transport-secret",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const grimoireContext = grimoire.hits.map((hit: any, index: number) => `[G${index + 1}] ${hit.title}\n${hit.excerpt}`).join("\n\n");
    const ancientContext = akst.hits.map((hit: any, index: number) => `[A${index + 1}] ${hit.title}\n${hit.excerpt}`).join("\n\n");
    const sacredLabeled = sacred.hits.map((hit: any, index: number) => ({ ...hit, label: `S${index + 1}` }));
    const sacredContext = (tier: string) => sacredLabeled
      .filter((hit: any) => hit.tier === tier)
      .map((hit: any) => `[${hit.label}] ${hit.title} — ${hit.section_heading} (${hit.standpoint})\n${hit.excerpt}`)
      .join("\n\n");
...
function serviceKey() {
  ...
  try {
    return Object.values(JSON.parse(Deno.env.get("SUPABASE_SECRET_KEYS") || "{}"))[0] as string | undefined;
  } catch {
    return undefined;
  }
}
...
function laws() {
  return ["Non-Imposition", "The Mirror", "Return to Wholeness", "Lineage-first", "No fabrication", "Source engine, not diagnostician"];
}

async function nativeEmbed(input: string): Promise<NativeEmbedding> {
  try {
    const model = new Supabase.ai.Session("gte-small");
    const raw = await model.run(input.slice(0, 4200), { mean_pool: true, normalize: true });
    const vector = Array.from(raw as ArrayLike<number>);
    if (vector.length !== 384 || !vector.every(Number.isFinite)) throw new Error(`invalid native embedding (${vector.length})`);
    return { vector, status: "ready", model: "gte-small", dimensions: 384 };
  } catch (error) {
    console.error("Oracle native embedding", error);
    return { vector: null, status: "unavailable", model: "gte-small", dimensions: 384 };
  }
}

async function getGrimoire(db: ReturnType<typeof createClient>, query: string, surface: Surface) {
  ...
  const hits = (data || []).map((row: any) => ({
    ...
    excerpt: [
      ...
      Array.isArray(row.tradition) && row.tradition.length && `Tradition: ${row.tradition.join(", ")}`,
...
async function getAncient(db: ReturnType<typeof createClient>, query: string, embedding: NativeEmbedding) {
  ...
  if (embedding.vector) {
    ...
    if (!error && data?.length) {
      return { status: "grounded", retrieval_mode: "native_vector", vector_status: "ready", hits: data.map(ancientHit) };
    }
  ...
  if (error) {
    ...
    return { status: "error", retrieval_mode: "lexical", vector_status: vectorStatus, hits: [] };
  }
  return { status: data?.length ? "grounded" : "empty", retrieval_mode: "lexical", vector_status: vectorStatus, hits: (data || []).map(ancientHit) };
}
...
async function getSacred(
  ...
  if (!rows.length) {
    const { data, error } = await db.rpc("search_sacred_writings_lexical", { query_text: query, match_count: 16 });
    if (error) {
  ...
  if (surface === "luminaria_client") rows = rows.filter((row: any) => String(row.tier) !== "B");
  rows = rows.slice(0, 8);
  ...
  hits.sort((left: any, right: any) => (tierRank[left.tier] ?? 9) - (tierRank[right.tier] ?? 9) || right.score - left.score);
...
function expandQuestion(question: string) {
  ...
  if (/sky|celestial|astrolog|moon|lunar/.test(lower)) terms.push("moon lunar sun solar planet planetary celestial sky");
  if (/love|relationship|romance/.test(lower)) terms.push("love venus heart relationship");
  if (/money|prosper|abundance/.test(lower)) terms.push("money prosperity abundance luck jupiter");
  if (/protect|banish|reversal|hex/.test(lower)) terms.push("protection ward banish reversal hex");
  if (/mother|maternal|woman|women|womb/.test(lower)) terms.push("mother womb woman women return wholeness");
  return terms.join(" ");
}
...
async function synthesize(system: string, question: string) {
  ...
  if (LOVABLE_API_KEY) {
    try {
      ...
      if (response.ok) return { text: (await response.json()).choices?.[0]?.message?.content || "", provider: "lovable" };
    } catch (error) {
  ...
  if (OPENAI_API_KEY) {
    try {
      ...
      if (response.ok) return { text: (await response.json()).choices?.[0]?.message?.content || "", provider: "openai" };
    } catch (error) {
...
function fallback(question: string, grimoire: any, ancient: any, sacred: any) {
  const parts = [`For “${question},” the Oracle found source evidence and is returning it without inventing a synthesis.`];
  if (sacred.hits.length) {
    const tiers = Object.entries(sacred.hits.reduce((acc: Record<string, number>, hit: any) => {
      acc[hit.tier] = (acc[hit.tier] || 0) + 1;
      return acc;
    }, {})).map(([tier, count]) => `${tier}:${count}`).join(", ");
    parts.push(`Sacred Writings returned ${sacred.hits.length} standpoint-gated sections [S]${tiers ? ` across tiers ${tiers}` : ""}.`);
  }
  if (ancient.hits.length) parts.push(`The Tier A ancient corpus returned ${ancient.hits.length} passage${ancient.hits.length === 1 ? "" : "s"} [A].`);
  else parts.push("No Tier A ancient passage matched this question.");
  if (grimoire.hits.length) parts.push(`The Grimoire returned ${grimoire.hits.length} correspondence${grimoire.hits.length === 1 ? "" : "s"} [G].`);
